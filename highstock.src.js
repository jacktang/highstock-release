...
var OHLCSeries = extendClass(seriesTypes.column, {
	...
	*/
				/*
		* Author haigui.Deng  2012-03-16 update color
		*/
		/*
		...
		*/
		...
		var black = 'black',
		seriesDownPointAttr  = merge(series.pointAttr),
		seriesDownPointAttr2 = merge(series.pointAttr),
		seriesDownPointAttr3 = merge(series.pointAttr);

		seriesDownPointAttr[''].fill       = upColor;
		seriesDownPointAttr[''].stroke     = upColor;
		seriesDownPointAttr.hover.fill     = upColor;
		seriesDownPointAttr.hover.stroke   = upColor;
		seriesDownPointAttr.select.fill    = upColor;
		seriesDownPointAttr.select.stroke  = upColor;

		seriesDownPointAttr2[''].fill      = color;
		seriesDownPointAttr2[''].stroke    = color;
		seriesDownPointAttr2.hover.fill    = color;
		seriesDownPointAttr2.hover.stroke  = color;
		seriesDownPointAttr2.select.fill   = color;
		seriesDownPointAttr2.select.stroke = color;

		seriesDownPointAttr3[''].fill      = black;
		seriesDownPointAttr3[''].stroke    = black;
		seriesDownPointAttr3.hover.fill    = black;
		seriesDownPointAttr3.hover.stroke  = black;
		seriesDownPointAttr3.select.fill   = black;
		seriesDownPointAttr3.select.stroke = black;

		each(series.points, function (point) {
			point.highColor = "black";
			point.closeColor = "black";
			point.lowColor = "black";
			if(point.open < point.high){
				point.highColor = "#F5000C";
			}else if(point.open > point.high){
				point.highColor = "#009500";
			}else{
				point.highColor = "black";
			}

			if(point.open < point.close){
				point.closeColor = "#F5000C";
			}else if(point.open > point.close){
				point.closeColor = "#009500";
			}else{
				point.closeColor = "black";
			}

			if(point.open < point.low){
				point.lowColor = "#F5000C";
			}else if(point.open > point.low){
				point.lowColor = "#009500";
			}else{
				point.lowColor = "black";
			}
			...
			if(point.open < point.close) {
				...
				point.pointAttr = seriesDownPointAttr3;
			}else{
				point.pointAttr = seriesDownPointAttr2;
			}
	...
	init: function () {
		...
		scroller.mouseMoveHandler = function (e) {
			...
			if (e.pageX !== 0) {
				...
				*/
				// drag left handle
				var wdWidth = scroller.navigatorOptions.wdWidth;
				if (scroller.grabbedLeft) {
					hasDragged = true;
					if(wdWidth && wdWidth > 0){
						if ((chartX - navigatorLeft + wdWidth) > 954) {
							chartX = 954 - wdWidth + navigatorLeft;
						}
						scroller.render(0, 0, chartX - navigatorLeft, chartX - navigatorLeft + wdWidth);
					}else{
				...
				// drag right handle
					...
					if(wdWidth && wdWidth > 0){
						...
						scroller.render(0, 0, chartX - wdWidth, chartX);
					}else{
